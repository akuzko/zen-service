module Excom
  module Plugins::Attributes
    Plugins.register :attributes, self
    ...
    private def resolve_args!(args)
      ...
      opts.each do |name, value|
        if attributes.key?(name)
          raise ArgumentError, "attribute #{name} has already been provided as parameter"
        end
    ...
    private def assert_valid_attributes!(actual)
      ...
      if unexpected.any?
        raise ArgumentError, "wrong attributes #{unexpected} given"
      end
